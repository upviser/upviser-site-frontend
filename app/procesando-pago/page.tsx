    ...
    if (tokenWs) {
      ...
      if (pay) {
        ...
        if (response.data.status === 'AUTHORIZED') {
          await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/pay/${pay._id}`, { state: 'Pago realizado', pay: pay })
          const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/services`)
          const service = JSON.parse(localStorage.getItem('service')!)
          service.step = response.data.find((ser: any) => ser._id === service.service).steps[response.data.find((ser: any) => ser._id === service.service).steps.find((step: any) => step._id === service.step) ? response.data.find((ser: any) => ser._id === service.service).steps.findIndex((step: any) => step._id === service.step) + 1 : 0]._id
          service.payStatus =
