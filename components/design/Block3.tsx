        ...
        <div className="text-center m-auto max-w-[1100px] w-full flex flex-col gap-8">
          ...
          </div>
          {
            content.info?.image && content.info.image !== ''
              ? <Image ref={imageRef} className={`${imageLoaded ? 'opacity-1' : 'opacity-0 translate-y-6'} transition-all duration-500 max-w-[700px] h-fit m-auto`} style={{ boxShadow: style.design === 'Sombreado' ? `0px 3px 20px 3px ${style.borderColor}10` : '', borderRadius: style.form === 'Redondeadas' ? `${style.borderBlock}px` : '', border: style.design === 'Borde' ? `1px solid ${style.borderColor}` : '' }} width={1400} height={1000} alt='Imagen slider prueba' src={content.info.image} />
              : ''
          }
        </div>
      </div>
